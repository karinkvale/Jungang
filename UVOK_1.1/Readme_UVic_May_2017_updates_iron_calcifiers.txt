...
of ocean biogeochemical tracers, GMDD: 
...
An index for biological tracers replaces hard-wired numbers in npzd_src.F and tracer.F:
ibion,ibiop,ibioz,ibiod,ibiono3,ibiodiaz,ibiodfe,ibiodetrfe

The sediment model is also activated for TMM applications. Modifications to the code were required for this:
1) Global river discharge now includes a common variable 'globaldisch'
2) A river discharge file can now be applied to the offline model. This file was created from the spinup file used to extract the matrices used in Kvale et al. (2017) GMD.
3) A new uvok_comm.h introduces a common block for sediment-related variables
4) A new npzd_ini routine is added to npzd_src.F to isolate biology from physics.
